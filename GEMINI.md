...
| 2. Cashier Accepts | `reserved` | Cashier Orders > Reservation (Waiting for kitchen / cooking) / Kitchen Board | `"RESERVED ORDER RECEIVED"` |
| 3. Kitchen Prep | `preparing` / `cooking` | Cashier Orders > Reservation (Cooking) / Kitchen Board | `"Preparing your food"` |
| 4. Table Received | `received` | Cashier Orders > Reservation (Received) | `"Completed"` |
...
3. **Accepting Reservation**: When clicking **Accept Reservation**, the order status is updated to `reserved`. It disappears from the Notification Bell, moves into **Cashier Orders > Reservation**, and is automatically forwarded to the Kitchen Display Board immediately. The old manual Check-in and Send-to-Kitchen steps are completely bypassed.
